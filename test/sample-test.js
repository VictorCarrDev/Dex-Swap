...
const SwapingBalancer = artifacts.require("SwapingBalancer");
...
contract("The balancer swaping contract bb", accounts => {
let instance

beforeEach(async () => {instance = await SwapingBalancer.new()})

  it("Should not explode ", async function() {
    const greeter = await SwapingBalancer.new();
    assert.ok(greeter.address);
  });
  ...
  it("Should not have any money left", async () =>{
    try {
      const transaction =  await instance.SwapMultiple([DAITOKEN,LINKTOKEN],[25,25],5,{value:web3.utils.toWei("1")})
      // uni = new ERC20(LINKTOKEN)
      // console.log("This is the log from the test %s",await uni.balanceOf(accounts[0]))
      assert.equal(0,await web3.eth.getBalance(instance.address))
     } catch (error) {
  ...
  it("Swap tokens until the percentage admit then return the extra money ", async () =>{
    try {
      const transaction =  await instance.SwapMultiple([UNITOKEN,DAITOKEN],[80,70],5,{value:web3.utils.toWei("1")})
      uni = new ERC20(UNITOKEN)
      console.log("This is the log from the test %s",await uni.balanceOf(accounts[0]))
      assert.equal(0,await web3.eth.getBalance(instance.address))
      assert.ok(transaction)
  ...
  it("Takes fee", async () => {
    let balance = await web3.eth.getBalance(accounts[0])
    let transaction =  await instance.SwapMultiple([UNITOKEN,DAITOKEN,LINKTOKEN],[20,25,40],5,{value:web3.utils.toWei("1"),from:accounts[1]})
    transaction =  await instance.SwapMultiple([UNITOKEN,DAITOKEN],[50,50],5,{value:web3.utils.toWei("1"),from:accounts[1]})
    transaction =  await instance.SwapMultiple([UNITOKEN,DAITOKEN],[80,70],5,{value:web3.utils.toWei("1"),from:accounts[1]})
    balance = parseInt(balance) + parseInt(web3.utils.toWei("0.85")) * 0.001 + parseInt(web3.utils.toWei("1"))* 0.001 + parseInt(web3.utils.toWei("0.8")) * 0.001
  ...
  it("I doenst run out of gas on aprox 10", async () =>{
    try {
      const transaction =  await instance.SwapMultiple([UNITOKEN,DAITOKEN,UNITOKEN,DAITOKEN,DAITOKEN,UNITOKEN,DAITOKEN,UNITOKEN,DAITOKEN],[10,10,10,10,10,10,10,10,10],5,{value:web3.utils.toWei("1")})
      uni = new ERC20(UNITOKEN)
      assert.equal(0,await web3.eth.getBalance(instance.address))
      assert.ok(transaction)
    } catch (error) {
